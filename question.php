...
class qtype_aitext_question extends question_graded_automatically_with_countback {
    ...
    public function build_full_ai_prompt($response, $aiprompt, $defaultmark, $markscheme): string {
        ...
        // Handle language behavior (admin toggle or prompt tag)
        if (strpos($aiprompt, '[[language=""]]') === false) {

            // Check if a specific [[language=xx]] tag is provided in the prompt.
            $langcode = null;
            if (preg_match('/\[\[language=([a-zA-Z]{2})\]\]/', $aiprompt, $matches)) {
                $langcode = $matches[1];
            }

            // Get admin-selected response language mode.
            $responselanguage = get_config('qtype_aitext', 'responselanguage');

            if ($langcode !== null) {
                // Explicit language tag (e.g. [[language=fr]]) overrides everything.
                $prompt .= ' Respond in ' . $langcode . ' language only.';
            } else if ($responselanguage === 'english') {
                // Always English (admin override)
                $prompt .= ' Respond in English only.';
            } else {
                // Default: same language as the student’s answer.
                $prompt .= ' Respond in the same language as the student’s answer. Do not translate.';
            }
        }
        return $prompt;
    }
